...
(def coll-d (vec (filter even? (range 1000))))
(def coll-e (vec (filter odd? (range 1000))))
(def coll-f (vec (range 1000)))
...
(deftest merge-colls-test
  ...
  (testing "should merge sorted collections."
    ...
    (is (= (merge-colls coll-a coll-b) (vec (concat coll-a coll-b))))
